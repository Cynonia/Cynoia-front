...
@Component({
      ...
      <div class="w-80 bg-white border-r border-gray-200 flex flex-col">
        ...
        <div class="p-4 border-b border-gray-200">
          <div class="flex items-center justify-between">
            ...
            <span class="text-xs text-gray-500">Communiquer</span>
          </div>
          <div class="mt-3">
            <div class="relative">
              ...
              <input type="text" [(ngModel)]="searchQuery" (input)="onSearchChange()" placeholder="Rechercher une conversation"
          ...
          <div *ngIf="activeTab === 'private'">
            ...
            <div *ngIf="conversations.length > 0" class="p-2">
              ...
              <div *ngFor="let conversation of conversations" 
                   ...
                   class="grid grid-cols-[auto,1fr,auto] items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors">
                <!-- Avatar -->
                <div class="relative">
                ...
                <!-- Main -->
                <div class="min-w-0">
                  <p class="text-sm font-medium text-gray-900 truncate">{{ conversation.participants[0].name }}</p>
                  <p class="text-xs text-gray-500 truncate" *ngIf="conversation.lastMessage as lm">{{ lm.content || '' }}</p>
                </div>
                <!-- Meta -->
                <div class="flex flex-col items-end gap-1">
                  <span class="text-[11px] text-gray-400">{{ formatTime(conversation.lastMessage?.timestamp) }}</span>
                  <span *ngIf="conversation.unreadCount > 0" class="bg-purple-600 text-white text-[10px] rounded-full px-2 py-0.5 min-w-[20px] text-center">
  ...
  <div class="flex-1 flex flex-col min-h-0 overflow-hidden">
        ...
        <!-- Conversation active -->
  <div *ngIf="activeConversation" class="flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          <!-- Messages -->
          <div class="flex-1 overflow-y-auto overflow-x-hidden p-4 space-y-4" #messagesContainer>
            <div *ngFor="let message of getActiveMessages()" class="flex" 
                 [class.justify-end]="isMyMessage(message)">
              <div [class]="getMessageClass(message)" [ngStyle]="getMessageStyle(message)" class="max-w-[92%] md:max-w-[80%] lg:max-w-[75%] break-words whitespace-pre-wrap overflow-hidden shadow-md">
                <p class="text-sm break-words whitespace-pre-wrap leading-relaxed">{{ message.content }}</p>
                <p class="text-[11px] text-gray-400 mt-1" [class.text-right]="isMyMessage(message)">{{ formatTime(message.timestamp) }}</p>
              </div>
            </div>
          </div>
          ...
          <div class="bg-white border-t border-gray-200 p-3">
            <form (ngSubmit)="sendMessage()" class="flex items-center gap-2">
              <button type="button" class="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-full" aria-label="Joindre un fichier">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button type="button" class="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-full" aria-label="Emoji">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
