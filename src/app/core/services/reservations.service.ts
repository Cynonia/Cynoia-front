...
export class ReservationsService {
  ...
  private loadReservations(): void {
    ...
    if (savedReservations) {
      try {
        const reservations = JSON.parse(savedReservations).map((reservation: any) => ({
          ...reservation,
  ...
  getReservationById(id: string): Reservation | undefined {
    return this.reservationsSubject.value.find(reservation => reservation.id === id);
  }
  ...
  addReservation(reservationData: Omit<Reservation, 'id' | 'createdAt' | 'updatedAt' | 'space'>): Reservation {
  ...
  updateReservationStatus(id: string, status: Reservation['status'], reason?: string): Reservation | null {
    const currentReservations = this.reservationsSubject.value;
